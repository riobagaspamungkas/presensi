...
<link rel="stylesheet" href="<?= base_url() ?>assets/vendors/css/vendor.bundle.addons.css">
                              ...
                              <tr>
                                ...
                                <td>
                                  ...
                                  if (!empty($row->id_absen)) { ?>
                                    <a href="<?= base_url() ?>pegawai/detail_kehadiran/<?= $row->id_absen;?>" class="btn btn-dark">Detail</a>
                                  <?php 
                                  } else {
                                    echo '-';
                                  }
  ...
  <script>
      ...
      if (document.getElementById("maps-absen")) {
        window.onload = function() {
          ...
          var geolocationMap = L.map("maps-absen", {
            center: [0.9034696201985035, 104.48766777189635],
            zoom: 15,
            zoomControl: false,
    ...
    $("#btn-absen-datang").click(function(e) {
      $.ajax({
        type: "POST",
        url: '<?= base_url();?>pegawai/absen/datang',
    ...
    $("#btn-absen-pulang").click(function(e) {
      $.ajax({
        type: "POST",
        url: '<?= base_url();?>pegawai/absen/pulang',
        data: {maps_absen: maps_absen}, // serializes the form's elements.
        beforeSend: function() {
          swal.fire({
              imageUrl: "<?= base_url(); ?>assets/images/ajax-loader.gif",
